...
import {
  ...
  Menu, User, Github, Linkedin, Twitter, Layout, Settings
} from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import toast, { Toaster } from 'react-hot-toast';

const API_BASE = import.meta.env.VITE_API_BASE || "https://vidheya.onrender.com";
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <div className="hidden md:flex items-center gap-8 text-sm font-semibold text-gray-500">
              ...
              <button className="flex items-center gap-2 hover:bg-gray-100 px-3 py-1.5 rounded-full transition-all">
                ...
                <span className="text-gray-700">Tanisha</span>
              </button>
            </div>
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-white border-t border-gray-100 p-4 absolute w-full shadow-lg">
            <div className="flex flex-col gap-4 text-gray-600 font-medium">
              ...
              <a href="#" className="hover:text-indigo-600">Calendar</a>
              <a href="#" className="hover:text-indigo-600">Reports</a>
        ...
        <div className="w-full max-w-2xl">
          ...
          <div className="bg-white/70 backdrop-blur-xl border border-white/50 rounded-[2rem] shadow-xl overflow-hidden ring-1 ring-black/5">
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-2">
                <div className="relative flex-1 group">
                  ...
                  <input
                    ...
                    className="w-full pl-11 pr-4 py-3 bg-white border border-gray-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-sm shadow-sm"
                ...
                <div className="flex bg-gray-100 p-1 rounded-xl">
                  {['all', 'active', 'completed'].map((f) => (
                    <button
                      key={f}
                      onClick={() => setFilter(f)}
                      className={`flex-1 px-4 py-2 rounded-lg text-xs font-bold capitalize transition-all ${filter === f
                ...
                {isLoading ? (
                  ...
                  filteredTodos.map(todo => (
                    <motion.div
                      ...
                      key={todo._id}
                      className={`group flex items-center justify-between p-4 mb-3 rounded-xl transition-all border select-none ${todo.completed ? "bg-indigo-50/50 border-indigo-100" : "bg-white border-transparent shadow-sm hover:border-indigo-100 hover:shadow-md"}`}
                    >
                      <div className="flex items-center gap-4 cursor-pointer flex-1" onClick={() => completeTodo(todo._id)}>
                        ...
                        <span className={`text-[15px] font-medium transition-all duration-300 ${todo.completed ? "text-gray-400 line-through decoration-gray-300" : "text-slate-700"}`}>
                          {todo.text}
                        </span>
                      </div>
                      <button onClick={(e) => { e.stopPropagation(); deleteTodo(todo._id); }} className="opacity-0 group-hover:opacity-100 focus:opacity-100 text-gray-300 hover:text-red-500 transition-all p-2 rounded-lg hover:bg-red-50">
                        <Trash2 size={18} />
                      </button>
                    </motion.div>
                  ))
            ...
            <div className="p-4 bg-white/80 border-t border-white/50 backdrop-blur-md">
              <button
                className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold text-md shadow-lg shadow-slate-200 hover:bg-slate-800 hover:scale-[1.01] active:scale-[0.98] transition-all flex items-center justify-center gap-2"
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          <div className="flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="text-center md:text-left">
              <h3 className="font-bold text-gray-700">Vidheya</h3>
              <p className="text-xs text-gray-500 mt-1">© 2026 Tanisha Sharma. All rights reserved.</p>
            </div>

            {/* Social Links */}
            <div className="flex items-center gap-6">
              <a href="#" className="text-gray-400 hover:text-indigo-600 transition-colors bg-white p-2 rounded-full shadow-sm hover:shadow-md">
                <Github size={20} />
              </a>
              <a href="#" className="text-gray-400 hover:text-blue-500 transition-colors bg-white p-2 rounded-full shadow-sm hover:shadow-md">
                <Twitter size={20} />
              </a>
              <a href="#" className="text-gray-400 hover:text-blue-700 transition-colors bg-white p-2 rounded-full shadow-sm hover:shadow-md">
                <Linkedin size={20} />
              </a>
            </div>
          </div>
            ...
            >
              <div className="p-6">
                <div className="flex justify-between items-center mb-6">
                  <h3 className="text-xl font-bold text-slate-800">Add Task</h3>
                  <button onClick={() => setPopupActive(false)} className="p-1 hover:bg-gray-100 rounded-full transition-colors">
                ...
                <input
                  type="text"
                  className="w-full bg-gray-50 border-2 border-transparent focus:border-indigo-500 focus:bg-white p-4 rounded-xl text-lg outline-none transition-all placeholder-gray-400 mb-6"
                ...
                <button
                  className="w-full py-3.5 rounded-xl font-bold bg-indigo-600 text-white hover:bg-indigo-700 shadow-lg shadow-indigo-200 transition-all active:scale-95 flex justify-center items-center gap-2 disabled:opacity-70 disabled:cursor-not-allowed"
                  onClick={addTodo}
                  disabled={isAdding}
                >
                  {isAdding ? <Loader2 className="animate-spin" size={20} /> : "Save Task"}
                </button>
              </div>
            </motion.div>
          </motion.div>
